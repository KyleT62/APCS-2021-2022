package com.company.ClassesAndArrays.ClientAndRollingDie;
...
public class ClientTester {
    ...
    public static void prefixMerge(Client [] list1, Client [] list2, Client[] result){
        ...
        for(int i = 0; i < result.length; i++){
            if(list1[i].compareClient(list2[incIndex1]) < 0){
                result[i] = list1[incIndex1];
                incIndex1++;
            }
            else if(list1[i].compareClient(list2[incIndex2]) > 0){
                result[i] = list2[incIndex2];
                incIndex2++;
            }
            else{
                result[i] = list1[incIndex2];
            }
        }
    }

package com.company.TwoDimensionalArrays.ScantArray;
...
public class ScantArray {
    ...
    public void removeColumn(int col){
        ...
        for (int k = entries.size()-1; k >= 0; k--){
            if (entries.get(k).getColumn() > col){
                int oldRow = entries.get(k).getRow();
                int newCol = entries.get(k).getColumn() - 1;
                int oldVal = entries.get(k).getValue();
                ScantArrayEntry ret = new ScantArrayEntry(oldRow, newCol, oldVal);
                entries.remove(entries.get(k));
                entries.add(ret);
            }
        }
        numColumns--;
    ...
    public static void main(String[] args){
        ...
        System.out.println(sa2.getValueAt(1,4));
        System.out.println(sa2.getValueAt(4,3));

package com.company.TwoDimensionalArrays.WordDuo;
...
public class WordDuoList {
    ...
    public WordDuoList(String[] words){
        ...
        for(int i = 0; i < words.length; i++){
            for(int k = i+1; k < words.length; k++) {
                WordDuo test = new WordDuo(words[i], words[k]);
                allDuos.add(test);
            }
        }
    }
    ...
    public int numMatches(){
        int count = 0;
        for(WordDuo matches : allDuos){
            if(matches.getFirst().equals(matches.getSecond())){
                count++;
            }
        }
    ...
    public void moveMatchesToTop(){
        for (int i = 0; i < allDuos.size(); i++){
            WordDuo matchTop = allDuos.get(i);
            if (matchTop.getFirst().equals(matchTop.getSecond())){
                allDuos.remove(i);
                allDuos.add(0, matchTop);
                i--;
            }
        }
    }
